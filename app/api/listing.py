...
def add_image(user: AUTH_ME, db: DB, image: UploadFile = File(...)):
    ...
    try:
        ...
        return ListingImageResponse(
            key=listing_image.key,
            url=f"{listing_image.storage_source}/{listing_image.storage_bucket}/{listing_image.key}{listing_image.extension}",
            extension=listing_image.extension,
        )
...
@router.post("/add-listing", description="Adds new listing")
...
@router.post("/update-listing", description="Update listing info")
